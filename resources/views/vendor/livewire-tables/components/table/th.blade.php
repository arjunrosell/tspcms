...
@php
    ...
    $direction = $column->hasField()
        ...
        : $component->getSort($column->getSlug()) ?? null;
@endphp

@if ($component->isTailwind())
    <th scope="col"
        {{ $attributes->merge($customAttributes)->class([
                'px-6 py-3 text-left bg-indigo-600 text-white text-xs font-medium whitespace-nowrap text-gray-500 uppercase tracking-wider dark:bg-gray-800 dark:text-gray-400' =>
            ...
            @else
                <button
                    ...
                    {{ $attributes->merge($customSortButtonAttributes)->class([
                            'flex items-center text-white space-x-1 text-left text-xs leading-4 font-medium text-gray-500 uppercase tracking-wider group focus:outline-none dark:text-gray-400' =>
                    ...
                    <span class="relative flex items-center">
                        @if ($direction === 'asc')
                            ...
                            <x-heroicon-o-chevron-down class="absolute w-3 h-3 opacity-0 group-hover:opacity-100" />
                        @elseif ($direction === 'desc')
                            <x-heroicon-o-chevron-down class="w-3 h-3 group-hover:opacity-0" />
                            <x-heroicon-o-x-circle class="absolute w-3 h-3 opacity-0 group-hover:opacity-100" />
                        @else
